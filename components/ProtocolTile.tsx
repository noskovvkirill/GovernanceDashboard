...
const ProtocolData = ({name, border, shadow}:{name?:string, border:string, shadow:string}) =>{
        ...
        {data.data.map((item: ProtocolData, i:number)=>{
            if(i===selected){
                return(
                    ...
                    boxShadow:shadow,
                    width:'100%',
                    height:'512px', overflow:'hidden',  border:border, borderRadius:'$2'}}
                    key={item.refId}>
                        <Box as='h3'>{item.title}</Box>
                            <Box as='p' css={{padding:'$2 0', maxWidth:'960px'}}>
                                {item.content}
                            </Box>
                    </Box>
                )
...
const ProtocolTile = ({ cname, minWidth, maxWidth, name, onClick, totalProposals, icons, totalVotes, uniqueVoters, selected}:Protocol) =>{
    ...
    const color = `hsl(${charToColor(name)}, 75%, 47%)`
    const border = `1px solid hsl(${charToColor(name)}, 75%, 63%)`
